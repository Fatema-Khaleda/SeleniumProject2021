...
public class Bcbs {
    ...
    @Test(priority = 4)// sendKeys(), clear()
    public void sendAndEnterByjsExecutor() throws InterruptedException {
		WebElement serchBox = driver.findElement(By.xpath("//input[@id='edit-keys']"));
		js.executeScript("arguments[0].click()", serchBox );
		Thread.sleep(2000);
		js.executeScript("arguments[0].value='Find a Doctor' ", serchBox); 
		Thread.sleep(2000);
   ...
   @Test(priority = 6) // text() & //action class method
   public void actionPipeline() throws InterruptedException {
		Actions actions=new Actions(driver); //mouse hover over 
    ...
    @Test(priority = 7) // contains(text()) & perform()
